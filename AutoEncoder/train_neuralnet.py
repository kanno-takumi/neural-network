...
#one_hot_label t_trainに関して「”true”の時one-hot表現 ”false”の時数値で扱う」
    ...
    t_train_0 = t_train[np.where(t_train==0)] #0のデータxß
    x_train_0 = x_train[np.where(t_train==0)] #0のデータt
    x_test_0 = x_test[np.where(t_test==0)]
    t_test_0 = t_test[np.where(t_test==0)]
...
#     f.write(str(x_train_0[0]))
    ...
    for i in range(iters_num): #1バッチに対して勾配法を用いる回数
        ...
        if i == iters_num- 1:
            ...
            make_img(network.predict(t_batch[0]),"t_out.png") #predictした状態のものを引数で与えている
